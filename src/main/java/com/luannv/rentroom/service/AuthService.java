...
public class AuthService {
    ...
    public Map<?, ?> validateUserRequest(BindingResult bindingResult, UserRegisterRequestDTO userRequestDTO) {
        ...
        bindingResult.getFieldErrors()
                .forEach(err -> errorsResponse.put(err.getField(),
                        ErrorCode.valueOf(err.getDefaultMessage()).getMessages()));
        // key: field, value: defaultMessages->ErrorCode CONST
        // why do not use <String, List<String>>, if has error, and map has key error,
        // then not catch error :)
        if (errorsResponse.get("username") == null
                && this.userRepository.existsByUsername(userRequestDTO.getUsername()))
        ...
        // errorsResponse.forEach((key, value) -> System.out.println(key + " " +
        // value));
    ...
    public UserResponseDTO addUser(UserRegisterRequestDTO userRequestDTO, MultipartFile multipartFile)
        ...
        if (this.securityUtils.getCurrentRole() != null
                && this.securityUtils.getCurrentRole().equalsIgnoreCase("ROLE_ADMIN")) {
            userEntity.setRole(this.roleRepository.findById(userRequestDTO.getRoleId()).orElseThrow());
        } else {
            userEntity.setRole(this.roleRepository.findById(3).orElseThrow());
        }
        ...
        if (multipartFile != null && !multipartFile.isEmpty()) {
            ...
            System.out.println(">>> loi, file null");
        }
